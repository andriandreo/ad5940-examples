/*!
 *****************************************************************************
 @file:    AD5940_SequencerBase(SEQgen).c
 @author:  Neo Xu (andriandreo's edit)
 @brief:   Basis from Basic usage of sequencer - Manual GPIO trigger removed 
           SEQ generator is ised for SEQID_0 instead
 -----------------------------------------------------------------------------
...
static AD5940Err AppSEQGen(void){
  ...
  //AD5940_SEQWriteReg(0x0000209C, (1L<<0)); /* Generate custom-interrupt 0 by manually write to AFEGENINTSTA reg. */
  ...
  //AD5940_SEQGenInsert(SEQ_WR(REG_AFE_AFEGENINTSTA, (1L<<0))); /* Generate custom-interrupt 0 by manually write to AFEGENINTSTA reg. */
  ...
  if(error == AD5940ERR_OK)
  {
    ...
    seqinfo0.pSeqCmd = pSeqCmd;
    seqinfo0.SeqLen = SeqLen;
    seqinfo0.WriteSRAM = bTRUE;

    /* Write command to SRAM */
    //AD5940_SEQCmdWrite(seqinfo0.SeqRamAddr, pSeqCmd, SeqLen); // DOES NOT WORK [!!!]
    AD5940_SEQInfoCfg(&seqinfo0); // WORKS [!!!]
  }
...
static int32_t AD5940PlatformCfg(void)
{
   ...
   */
  gpio_cfg.FuncSet = GP0_INT|GP4_SYNC;
  gpio_cfg.OutputEnSet = AGPIO_Pin0|AGPIO_Pin4;
  gpio_cfg.OutVal = 0;
...
void AD5940_Main(void)
{
  ...
  seqinfo1.SeqRamAddr = SEQ0ADDR + seqinfo0.SeqLen;
  ...
  seqinfo2.SeqRamAddr = seqinfo1.SeqRamAddr + seqinfo1.SeqLen;
  ...
  seqinfo3.SeqRamAddr = seqinfo2.SeqRamAddr + seqinfo2.SeqLen;
  ...
  /* Test MMR trigger for SEQ0*/
  printf("\nTest2: trigger sequence0 manually by register write.\n");
  AD5940_SEQMmrTrig(SEQID_0); /* Trigger sequence0 manually. */
  /* Wait until CUSTMINT0 is set. We generate this interrupt in SEQ0 */
  while(AD5940_INTCTestFlag(AFEINTC_1, AFEINTSRC_CUSTOMINT0) == bFALSE);  /* Test INTC1, we enabled all interrupts in INTC1. */
  AD5940_INTCClrFlag(AFEINTSRC_CUSTOMINT0);
  printf("sequence0 has been executed\n");
  AD5940_INTCClrFlag(AFEINTSRC_ALLINT);
